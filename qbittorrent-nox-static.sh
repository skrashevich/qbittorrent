...
set_module_urls() {
	...
	if [[ ! "${what_id}" =~ ^(alpine)$ ]]; then
		...
		#bison_url="https://mirrors.kernel.org/gnu/bison/bison-${bison_version}.tar.gz"
		bison_url="https://mirrors.kernel.org/gnu/bison/$(grep -Eo 'bison-([0-9]{1,3}[.]?)([0-9]{1,3}[.]?)([0-9]{1,3}?)\.tar.gz' <(curl https://mirrors.kernel.org/gnu/bison/) | sort -V | tail -1)"
	fi
	...
	if [[ ! "${what_id}" =~ ^(alpine)$ ]]; then
		...
		#gawk_url="https://mirrors.kernel.org/gnu/gawk/gawk-${gawk_version}.tar.gz"
		gawk_url="https://mirrors.kernel.org/gnu/gawk/$(grep -Eo 'gawk-([0-9]{1,3}[.]?)([0-9]{1,3}[.]?)([0-9]{1,3}?)\.tar.gz' <(curl https://mirrors.kernel.org/gnu/gawk/) | sort -V | tail -1)"
	fi
	...
	if [[ ! "${what_id}" =~ ^(alpine)$ ]]; then
		if [[ "${what_version_codename}" =~ ^(jammy)$ ]]; then
			...
			# glibc_url="https://mirrors.kernel.org/gnu/libc/glibc-${glibc_version}.tar.gz"
			# glibc_url="https://mirrors.kernel.org/gnu/glibc/$(grep -Eo 'glibc-([0-9]{1,3}[.]?)([0-9]{1,3}[.]?)([0-9]{1,3}?)\.tar.gz' <(curl https://mirrors.kernel.org/gnu/glibc/) | sort -V | tail -1)"
			glibc_url="https://mirrors.kernel.org/gnu/libc/glibc-2.37.tar.gz" # pin to the same version for this OS otherwise we get build errors
		else
			glibc_url="https://mirrors.kernel.org/gnu/libc/glibc-2.31.tar.gz" # pin to the same version for this OS otherwise we get build errors
		fi
	fi
	...
	iconv_url="https://mirrors.kernel.org/gnu/libiconv/$(grep -Eo 'libiconv-([0-9]{1,3}[.]?)([0-9]{1,3}[.]?)([0-9]{1,3}?)\.tar.gz' <(curl https://mirrors.kernel.org/gnu/libiconv/) | sort -V | tail -1)"
